...
const selectOptions = async (crud, table) => {
    ...
    if (crud === "Create" && table === "Employees") {
        ...
        crud === "Read" &&
        table !== "Employees by Manager" &&
        table !== "Departments with Total Budget"
    ) {
        fields = {};
        ...
        COALESCE(SUM(salary), 0) AS 'total_budget'
        `;
    ...
    await runMainQuery(crud, table, fields, where, columns);
};
...
const promptUpdateDepartmentsFields = async () => {
    ...
    const query5 = await new Query(
        "Read Where",
        "Departments",
        [{}],
        `department_id=${answer1.department_id}`,
        "*"
    ).buildQuery();
...
const promptUpdateRolesFields = async () => {
    ...
    const query4 = await new Query(
        "Read Where",
        "Roles",
        [{}],
        `role_id=${answer1.role_id}`,
        "*"
    ).buildQuery();
...
const runMainQuery = async (crud, table, fields, where, columns) => {
    ...
    if (
        crud === "Read" ||
        (crud === "Read Where" && query2.fields[0] === "Employees by Manager") ||
        crud === "Read Total Budget"
    ) {
